...
%%
lhs_samples1 = lhs_samples((lhs_samples(:,1)<0.3|lhs_samples(:,1)>0.7)|...
    (lhs_samples(:,2)<0.3|lhs_samples(:,2)>0.7),:);
% lhs_samples2 = lhs_samples1(lhs_samples1(:,2)<0.3|lhs_samples1(:,2)>0.7,:);
% Map the LHS samples to the specified range
lhs_samples = lhs_samples1;
...
% Specify the number of points
numPointstest1 = floor(sqrt(size(lhs_samples1, 1)));
...
[sMap]= modifiedsom_lininit1(sData,'lattice','hexa','msize',[numPointstest1,numPointstest1]);
sMap1 = sMap;

%% Training SOM
[sMap,sTrain] = modifiedsom_batchtrain(sMap,sData,'sample_order','ordered', ...
    'radius_ini',1.0, 'radius_fin', 0.75,'trainlen',200);
...
%% Density matrix
sMap_umatrix = sMap;
[p_mat, dist]= som_density_mat(sMap,data1);
...
%% iSOM Grid in function space  
figure(17)
som_grid(sMap,'coord',sMap.codebook,'label',sMap.labels,'labelcolor','c','labelsize',5, 'marker','o','MarkerColor','k'...
    ,'MarkerSize',7,'linecolor', 'k');
hold on, scatter3(data1(:,1),data1(:,2),data1(:,3),20,'ro','filled');
...
[sMap1,sTrain] = modifiedsom_batchtrain(sMap1,sData,'sample_order','ordered', ...
    'radius_ini', 1.12, 'radius_fin', 0.75, 'trainlen',200);
